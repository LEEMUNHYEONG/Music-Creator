...
20. 완료: 씬 품질 요약에서 누락 유형별 필터/하이라이트 표시
21. 완료: 씬 품질 요약 필터 선택 상태를 카드 요약/안내 문구에 연결
22. 완료: 씬 개요 확정 전 확인 필요 항목이 남아 있을 때 부드러운 확인 알림 추가
23. 완료: 씬 개요 확정 전 확인 알림에서 첫 문제 씬 이동 안내를 더 명확히 표시
24. 완료: 실제 Chrome 렌더링 기준 MV 6단계 씬 카드 시각 점검 자동화
25. 완료: MV 6단계 개선 내역을 사용자 시나리오 기준으로 최종 통합 점검
26. 완료: 실제 사용자 데이터로 MV 6단계 작업 흐름을 운영 전 리허설할 체크리스트 작성
27. 완료: 실제 프로젝트 리허설 준비 상태와 저장 프로젝트 데이터 탐색 결과 기록
28. 완료: 실제 Chrome localStorage 대표 프로젝트 기준 MV 6단계 운영 전 리허설 자동화
29. 완료: MV 6단계 릴리스 후보 판정 문서 작성
30. 완료: 실제 프로젝트 수동 리허설 기록지와 로컬 서버 실행 스크립트 추가
31. 완료: 자동 테스트 통과 후 수동 리허설 서버를 여는 `rehearse:mv` 명령 추가
32. 완료: 수동 리허설 시작 전 서버 응답과 기준 커밋 기록
33. 완료: 인앱 브라우저에서 앱 로드와 저장 프로젝트 없음 상태 기록
34. 완료: 샘플 프로젝트 생성 후 1→2단계 이동 확인 및 진행바 마크업 보호 테스트 추가
35. 완료: 진행바 5/6단계 아이콘 클릭 차단 수정 후 6단계 진입 확인
36. 완료: 5→6단계 이동 시 최종 가사 부재 대체 입력 처리 추가
37. 완료: Gemini API 키가 없어도 MV 프롬프트 탭 작업공간을 열 수 있게 보강
38. 완료: API 키 없이도 6단계 MV 씬 카드가 로컬 기본 방식으로 생성되도록 보강
39. 완료: 프로젝트 복원 중 객체형 보컬 파트 지정값으로 인한 렌더링 오류 방지
40. 완료: 6단계 MV 프롬프트 탭 씬 편집값이 저장 후 새로고침/프로젝트 재로드에서도 복원되도록 보강

다음 추천 순서:

1. MV 개선 안정화 마감 태그 기준으로 변경 파일/테스트/브라우저 리허설 결과를 정리
