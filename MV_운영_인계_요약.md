...
자동 테스트: npm run test:mv -> PASS (97 checks)
```
...
12. 프로젝트 저장 전 `marketing.mv` 진단 요약 로그, 수동 진단 버튼, 저장 전/후 씬 비교 제공
13. 단일 프로젝트 JSON 내보내기/가져오기와 `marketing.mv` 정규화 보존 지원
14. 씬별 프롬프트 길이, 금지어, 중복 표현, 비어 있는 장소/카메라 값 품질 점검 제공
15. 씬 간 배경/구도/카메라 반복 패턴 품질 경고 제공
16. 인물 외형 일관성 키워드 누락 품질 경고 제공
17. 이미지/영상/TXT 내보내기 결과에 최종 품질 체크리스트 포함
18. 자동 테스트, Chrome 런타임, 샘플 프로젝트 리허설 문서화 완료
